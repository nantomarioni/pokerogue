...
import { dropAllSpeedOverrides } from "#system/speed-overrides";
...
export function setSetting(setting: string, value: number): boolean {
  ...
  switch (Setting[index].key) {
    case SettingKeys.Game_Speed:
      // An explicit player choice must never be undone by a pending fork speed override
      dropAllSpeedOverrides();
